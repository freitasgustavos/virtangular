...
class Configuration
{
    ...
    public static function config($configFile = null)
    {
        ...
        $configDistFile = $dir . DIRECTORY_SEPARATOR . 'codeception.dist2.yml';
    ...
    {
        $suites = Finder::create()->files()->name('*.{suite,suite.dist2}.yml')->in(self::$dir . DIRECTORY_SEPARATOR . self::$testsDir)->depth('< 1');
        self::$suites = [];

        /** @var SplFileInfo $suite */
        foreach ($suites as $suite) {
            preg_match('~(.*?)(\.suite|\.suite\.dist2)\.yml~', $suite->getFilename(), $matches);
    ...
    protected static function loadEnvConfigs($path)
    {
        ...
        foreach ($envFiles as $envFile) {
            preg_match('~^(.*?)(\.dist2)?\.yml$~', $envFile->getFilename(), $matches);
            $envs[] = $matches[1];
        }
        ...
        foreach ($envs as $env) {
            $envConfig[$env] = [];
            foreach (['.dist2.yml', '.yml'] as $suffix) {
    ...
    /**
     * Loads config from *.dist2.suite.yml and *.suite.yml
     *
     * @param $suite
     * @param $path
     * @param $settings
    ...
    {
        $suiteDistconf = self::getConfFromFile(self::$dir . DIRECTORY_SEPARATOR . $path . DIRECTORY_SEPARATOR . "$suite.suite.dist2.yml");
